...
pub fn get_ways_going_through_node_id<'a>(osm:&'a Osm, node_id:i64)-> Vec<&'a Way>
{
    osm.ways.iter()
            .filter(move |(_,w)| w.nodes.iter().any(|n| *n == node_id))
            .map(|(_,v)| v)
            .collect()
}

fn get_node_ids_in_ways<'a>(ways: &Vec<&'a Way>) -> impl Iterator<Item = i64>
{
    ways.into_iter().map(|w| w.nodes.iter()).flatten().map(|s| s.clone()).collect::<Vec<i64>>().into_iter().unique()
}
...
pub fn get_reachable_ways_for_node_id<'a>(osm:&'a Osm, node_id:i64)-> Vec<&'a Way>
{
